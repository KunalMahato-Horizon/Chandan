    ...
    <div className="relative min-h-screen bg-[#080808] text-white flex items-center justify-center overflow-hidden selection:bg-blue-600 pt-20 lg:pt-0">
      ...
      <main className="container mx-auto px-4 sm:px-6 lg:px-12 relative z-10 py-8 sm:py-12 lg:py-20">
        <div className="flex flex-col lg:flex-row items-center gap-8 sm:gap-12 lg:gap-16 xl:gap-24">
          ...
          <motion.div variants={container} initial="hidden" animate="show" className="flex-1 w-full space-y-6 sm:space-y-8 lg:space-y-10">
            <motion.div variants={item} className="space-y-3 sm:space-y-4 lg:space-y-6">
              ...
              <div className="space-y-1 sm:space-y-2">
                ...
                <div className="flex items-center gap-2 sm:gap-3 pt-2 sm:pt-4">
                  <Sparkles size={10} className="text-blue-500 animate-pulse sm:w-3 sm:h-3 lg:w-4 lg:h-4" />
            ...
            <motion.p variants={item} className="text-zinc-400 text-sm sm:text-base lg:text-lg xl:text-xl max-w-sm leading-relaxed border-l-2 border-zinc-800 pl-3 sm:pl-4 lg:pl-6">
            ...
            <motion.div variants={item} className="flex flex-wrap gap-1.5 sm:gap-2 pt-2 sm:pt-4">
              {["PR", "AE", "DR", "C4D"].map((tool) => (
                <span key={tool} className="text-[7px] sm:text-[8px] lg:text-[9px] font-bold font-mono px-1.5 sm:px-2 lg:px-3 py-1 bg-white/5 border border-white/10 rounded-md text-zinc-400 hover:text-white hover:border-white/30 transition-colors cursor-default">
                  {tool}
                </span>
              ))}
            </motion.div>
          ...
          >
            <div className="relative group">
              ...
              {/* Float Badges - Hidden on mobile and tablet, visible on desktop */}
      ...
      {/* Atmospheric Background Text - Hidden on mobile */}
      <div className="absolute bottom-[-2%] right-[-2%] text-[15vw] sm:text-[18vw] font-black text-white/[0.015] pointer-events-none select-none uppercase tracking-tighter italic hidden md:block">
        CHANDAN
      </div>
    </div>
  );
